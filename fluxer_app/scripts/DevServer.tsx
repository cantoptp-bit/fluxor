...
async function main(): Promise<void> {
	...
	// Use config.test.json for local dev when FLUXER_CONFIG is not set (e.g. frontend-only dev)
	if (!process.env.FLUXER_CONFIG) {
		const defaultConfig = path.join(projectRoot, '..', 'config', 'config.test.json');
		try {
			await stat(defaultConfig);
			process.env.FLUXER_CONFIG = defaultConfig;
			console.log('FLUXER_CONFIG not set; using', process.env.FLUXER_CONFIG);
		} catch {
			console.error(
				'FLUXER_CONFIG is not set and config/config.test.json was not found. Set FLUXER_CONFIG to a config JSON path (e.g. config/config.test.json from repo root) or run from repo root with scripts that set it.',
			);
			process.exit(1);
		}
	}
