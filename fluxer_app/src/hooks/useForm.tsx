...
export function useForm({initialValues = {}, onSubmit}: UseFormOptions) {
	...
	const getValue = useCallback((fieldName: string): string => fieldsRef.current[fieldName]?.value ?? '', []);

	const getError = useCallback((fieldName: string): string | undefined => fieldsRef.current[fieldName]?.error, []);
