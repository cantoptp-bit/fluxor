...
import {getSafeGuildChannels, type GuildReadyData} from '@app/types/gateway/GatewayGuildTypes';
...
class ChannelStore {
	...
	handleGuildCreate(guild: GuildReadyData): void {
		...
		const guildChannels = getSafeGuildChannels(guild);
	...
	/**
	 * Fetch guild channels from the REST API and merge into the store.
	 * Used when the gateway did not include channels (e.g. Node gateway READY).
	 */
	async fetchGuildChannelsFromApi(guildId: string): Promise<Array<ChannelRecord>> {
		try {
			const response = await http.get<Array<Channel>>({
				url: Endpoints.GUILD_CHANNELS(guildId),
			});
			const channels = response?.body ?? [];
			if (Array.isArray(channels) && channels.length > 0) {
				this.handleChannelUpdateBulk({channels});
				for (const ch of channels) {
					if (ch?.id) PermissionStore.handleChannelUpdate(ch.id);
				}
			}
		} catch {
			// Leave store unchanged on failure
		}
		return this.getGuildChannels(guildId) as Array<ChannelRecord>;
	}
