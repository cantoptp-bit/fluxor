...
class RuntimeConfigStore {
	...
	private async resolveBootstrapEndpoint(): Promise<string> {
		...
		// On Vercel, if fallback is localhost or same-origin we don't throw; we return same-origin /api so
		// discovery is attempted and then minimal config fallback can run. That way the site always loads
		// and shows the login page with "Could not reach the backend" instead of a full-page error.
		if (isVercel && fallback) {
			try {
				const fallbackUrl = fallback.startsWith('/') ? origin + fallback : fallback;
				const u = new URL(fallbackUrl);
				if (u.hostname === 'localhost' || u.hostname === '127.0.0.1' || u.origin === origin) {
					return origin + '/api';
				}
			} catch {
				// invalid URL; return fallback as-is
			}
		}
		return fallback;
	}
	...
	/** Fewer retries on localhost so the app loads with minimal config quickly when no backend is running. */
	private static readonly BOOTSTRAP_RETRY_ATTEMPTS_LOCALHOST = 2;
	...
	private async initialize(): Promise<void> {
		try {
			...
			try {
				...
				const bootstrapUrl = bootstrapEndpoint.startsWith('/') ? origin + bootstrapEndpoint : bootstrapEndpoint;
				let isLocalhostOrSameOrigin = false;
				try {
					const u = new URL(bootstrapUrl);
					isLocalhostOrSameOrigin =
						u.hostname === 'localhost' ||
						u.hostname === '127.0.0.1' ||
						u.origin === origin;
				} catch {
					// relative or invalid URL; treat as same-origin for minimal config fallback
					if (bootstrapEndpoint.startsWith('/')) isLocalhostOrSameOrigin = true;
				}

				// On Vercel, if discovery fails (CORS, backend down, or FLUXER_PUBLIC_DOMAIN not set),
				// always use minimal config so the site loads and shows the login form with "Could not reach
				// the backend" hint. Login will then show a clear network error if the user tries without a backend.
				if (isVercel) {
					runInAction(() => {
				...
				// On localhost/same-origin (e.g. local dev without backend), use minimal config so the app loads
				// and shows the login page with "Could not reach the backend" hint instead of a blank or error-only screen.
				if (isLocalhostOrSameOrigin) {
					runInAction(() => {
						const minimal = this.buildMinimalInstanceFromBootstrapUrl(bootstrapEndpoint);
						this.updateFromInstance(minimal);
						this.usedMinimalConfig = true;
	 ...
	 * On localhost we use fewer retries so minimal config fallback kicks in quickly when no backend is running.
	 */
	private async connectToEndpointWithRetry(bootstrapEndpoint: string): Promise<void> {
		let isLocalhostOrSameOrigin = false;
		if (typeof window !== 'undefined') {
			try {
				const url = bootstrapEndpoint.startsWith('/')
					? new URL(window.location.origin + bootstrapEndpoint)
					: new URL(bootstrapEndpoint);
				const origin = window.location.origin;
				isLocalhostOrSameOrigin =
		...
		const maxAttempts = isLocalhostOrSameOrigin
			? RuntimeConfigStore.BOOTSTRAP_RETRY_ATTEMPTS_LOCALHOST
			: RuntimeConfigStore.BOOTSTRAP_RETRY_ATTEMPTS;
