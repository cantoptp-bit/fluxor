...
class GatewayConnectionStore {
	...
	private createGatewaySocket(token: string, properties: GatewaySocketProperties, generation: number): GatewaySocket {
		...
		logger.debug(`Using gateway compression: ${compression}`);
	...
	/** Force an immediate reconnect attempt (e.g. after "Having trouble connecting" or post-restart). */
	retryConnection(): void {
		if (this.socket) {
			this.socket.reset(true);
		} else {
			void import('@app/actions/AuthenticationActionCreators').then(({ensureSessionStarted}) =>
