...
function shouldTreatGuildCreateAsUnavailable(data: GuildReadyData): boolean {
	// Only treat as unavailable when the gateway explicitly marks it so.
	// Node gateway (and minimal READY payloads) may omit channels/members/roles/emojis;
	// we still accept the guild and fetch channels from the API when missing.
