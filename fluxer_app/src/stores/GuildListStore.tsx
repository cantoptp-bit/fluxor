...
class GuildListStore {
	...
	getOrganizedGuildList(): Array<OrganizedItem> {
		...
		const guildIdsInFolders = new Set(guildFolders.flatMap((folder) => folder.guildIds));
		...
		// Show guilds not in any folder (e.g. newly created, or when guild_folders is empty)
		const orphanGuilds = this.guilds
			.filter((guild) => !guildIdsInFolders.has(guild.id))
			.sort((a, b) => a.name.localeCompare(b.name));
		for (const guild of orphanGuilds) {
			result.push({type: 'guild', guild});
		}
	...
	/**
	 * Returns the organized guild list for one type only (servers or communities).
	 * Order is preserved from the full list so each section respects your arrangement.
	 * Guild types are synced from the API (GET /users/@me/guilds) after READY so
	 * server vs community is correct even when the gateway omits guild_type.
	 */
	getOrganizedGuildListByType(type: 'server' | 'community'): Array<OrganizedItem> {
		const items = this.getOrganizedGuildList();
		const result: Array<OrganizedItem> = [];
		for (const item of items) {
			if (item.type === 'guild') {
				if (item.guild.guildType === type) result.push(item);
				continue;
			}
			const matchingGuilds = item.guilds.filter((g) => g.guildType === type);
			if (matchingGuilds.length > 0) {
				result.push({type: 'folder', folder: item.folder, guilds: matchingGuilds});
			}
		}
		return result;
	}
