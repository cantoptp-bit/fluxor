...
import {getSafeGuildVoiceStates, type GuildReadyData} from '@app/types/gateway/GatewayGuildTypes';
...
export class VoiceStateGatewayHandler {
	...
	handleConnectionOpen(guilds: Array<GuildReadyData>): void {
		...
		logger.debug('Initialized voice states from connection open', {
	...
	handleGuildCreate(guild: GuildReadyData): void {
		const voiceStates = getSafeGuildVoiceStates(guild);
	...
	handleGuildDelete(deletedGuildId: string): void {
		...
		logger.debug('Removed all voice states for deleted guild', {guildId: deletedGuildId});
	}
}
