...
export const RootComponent: React.FC<{children?: React.ReactNode}> = observer(({children}) => {
	...
	useEffect(() => {
		...
		if (shouldBypassGateway) {
			if (isAuth) {
				// Let user stay on login page so they can log in again or use another account (e.g. after stale session / gateway not ready)
	...
	// Repair account display when stored userData or UserStore shows mock (e.g. from gateway READY with mock)
	useEffect(() => {
		if (!isAuthenticated) return;
		const userId = AccountManager.currentUserId;
		if (!userId) return;
		const account = AccountManager.currentAccount;
		const storeUsername = currentUser?.username;
		const accountUsername = account?.userData?.username;
		const isMock = storeUsername === 'MockUser' || accountUsername === 'MockUser';
		if (!isMock) return;
		void AuthenticationActionCreators.refreshAccountUserDataIfMock(userId);
	}, [isAuthenticated, AccountManager.currentUserId, currentUser?.username, AccountManager.currentAccount?.userData?.username]);
