...
export type GuildType = 'server' | 'community';
...
export class GuildRecord {
	...
	constructor(guild: GuildInput, options?: GuildRecordOptions) {
		...
		this.guildType = this.normalizeGuildType(guild);
	...
	private normalizeGuildType(guild: GuildInput): GuildType {
		const value = this.isGuildInput(guild) ? guild.guild_type : guild.guildType;
	...
	static fromGuildReadyData(guildData: GuildReadyData | Record<string, unknown>, instanceId?: string): GuildRecord {
		// Support both GuildReadyData (properties wrapper + roles) and flat API format (Node gateway)
		const properties = (guildData as {properties?: Record<string, unknown>}).properties ?? guildData;
		const guildId = typeof (properties as {id?: string}).id === 'string' ? (properties as {id: string}).id : String((properties as {id?: unknown}).id ?? '');
		const rolesArray = Array.isArray((guildData as {roles?: unknown}).roles) ? (guildData as {roles: Array<{id: string; name?: string; color?: number; position?: number; hoist_position?: number | null; permissions: string; hoist?: boolean; mentionable?: boolean}>}).roles : [];

		const rolesMap = rolesArray.reduce<Record<string, GuildRoleRecord>>(
			(acc, role) => ({
		...
		// Ensure @everyone role exists (required when API returns flat guild without roles)
		if (!rolesMap[guildId]) {
			rolesMap[guildId] = new GuildRoleRecord(guildId, {
				id: guildId,
				name: '@everyone',
				color: 0,
				position: 0,
		...
		const roles = Object.freeze(rolesMap);
		const joinedAt = (guildData as {joined_at?: string}).joined_at ?? null;
		const unavailable = (guildData as {unavailable?: boolean}).unavailable ?? false;
		...
		return new GuildRecord(
			{
				...
				guildType: guild.guild_type ?? this.guildType,
