...
export async function ensureSessionStarted(): Promise<void> {
	...
	try {
		logger.debug('Ensuring session is started');
...
/** Fetch current user profile so we can show correct username in account list (avoids "Unknown user#0000" before gateway READY). */
async function fetchUserDataForToken(token: string): Promise<UserData | undefined> {
	try {
		const res = await http.get<{username?: string; discriminator?: string | number; email?: string | null; avatar?: string | null}>({
			url: Endpoints.USER_ME,
			skipAuth: true,
			headers: withPlatformHeader({ Authorization: `Bearer ${token}` }),
		});
		const u = res.body;
		const username = u?.username ?? undefined;
		const discriminator =
			typeof u?.discriminator === 'number'
				? String(u.discriminator).padStart(4, '0')
				: (u?.discriminator as string | undefined) ?? '0000';
		if (username) {
			return { username, discriminator, email: u?.email ?? undefined, avatar: u?.avatar ?? undefined };
		}
		return undefined;
	} catch (err) {
		logger.warn('Could not fetch user data for new login', err);
		return undefined;
	}
}

/** Returns true if account had mock/stale userData and was updated with real data from API. */
export async function refreshAccountUserDataIfMock(userId: string): Promise<boolean> {
	const account = AccountManager.orderedAccounts.find((a) => a.userId === userId);
	if (!account?.token) return false;
	const isMock =
		account.userData?.username === 'MockUser' || (!account.userData?.username && account.token);
	if (!isMock) return false;
	const userData = await fetchUserDataForToken(account.token);
	if (!userData) return false;
	AccountManager.updateAccountUserData(userId, userData);
	await accountStorage.updateAccountUserData(userId, userData);
	// Update UserStore so profile/header show real username (they read from UserStore.currentUser, not account userData)
	if (AccountManager.currentUserId === userId) {
		UserStore.handleUserUpdate({
			id: userId,
...
export async function completeLogin({
	...
	if (userId && token) {
		const resolvedUserData = userData ?? (await fetchUserDataForToken(token));
		await AccountManager.switchToNewAccount(userId, token, resolvedUserData, false);
	} else {
