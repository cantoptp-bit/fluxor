...
import '@app/suppressReactDevtoolsWarning';
...
// Show loading immediately so the page is never blank (e.g. while bootstrap runs or if API is slow)
...
/** Send errors to the debug terminal at /debug for development troubleshooting */
function initDebugErrorReporter(): void {
	const sendToDebug = (payload: Record<string, unknown>): void => {
		try {
			fetch('/_debug/log', {
				method: 'POST',
				...
				body: JSON.stringify({
					level: 'error',
					service: 'fluxer-app',
					msg: payload.msg ?? 'Uncaught error',
