	...
	return (
		<div className={styles.container} ref={listRef} style={{width: 'var(--layout-header-popout-width)'}}>
			...
			{canAccessGuildSettings && (
				<GuildHeaderPopoutItem
					icon={GearIcon}
					title={guild.guildType === 'server' ? t`Server Settings` : t`Community Settings`}
			...
			<GuildHeaderPopoutItem
				icon={UserCircleIcon}
				title={guild.guildType === 'server' ? t`Edit Server Profile` : t`Edit Community Profile`}
				onClick={() => {
			...
			{!guild.isOwner(AuthenticationStore.currentUserId) && (
				<GuildHeaderPopoutItem
					...
					title={guild.guildType === 'server' ? t`Leave Server` : t`Leave Community`}
				/>
			)}
		</div>
	);
