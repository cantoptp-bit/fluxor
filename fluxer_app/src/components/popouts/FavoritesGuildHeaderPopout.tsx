...
import {FolderPlusIcon, PlusCircleIcon, StarIcon} from '@phosphor-icons/react';
...
export const FavoritesGuildHeaderPopout = observer(() => {
	...
	return (
		<div className={styles.container} ref={listRef} style={{width: 'var(--layout-header-popout-width)'}}>
			...
			<GuildHeaderPopoutItem
				icon={StarIcon}
				title={t`Favorite Friends`}
				onClick={() => ModalActionCreators.push(modal(() => <AddFavoriteFriendsModal />))}
			/>
			<GuildHeaderPopoutItem
				icon={FolderPlusIcon}
