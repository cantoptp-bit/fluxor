...
import styles from '@app/components/pages/LoginPageV2.module.css';

const LoginPageV2Content = observer(function LoginPageV2Content() {
	...
	const redirectPath = isDesktopHandoff ? undefined : rawRedirect ?? undefined;
	...
	return (
		<div className={styles.container}>
			<AuthLoginLayout
				...
				registerLink={
					<AuthRouterLink to="/register" search={registerSearch} className={styles.footerLink}>
...
const LoginPageV2MFA = observer(function LoginPageV2MFA() {
	const location = useLocation();
	const params = useMemo(() => new URLSearchParams(location.search), [location.search]);
	...
	const handleHandoffContinueInBrowser = useCallback(async () => {
		if (!handoff.successPayload) return;
		await AuthenticationActionCreators.completeLogin(handoff.successPayload);
		AuthenticationActionCreators.clearMfaTicket();
		RouterUtils.replaceWith(redirectTo || '/');
	}, [handoff.successPayload, redirectTo]);
	...
	if (
		...
		return (
			<HandoffCodeDisplay
				...
				onContinueInBrowser={handleHandoffContinueInBrowser}
			/>
		);
	}
...
const LoginPageV2Container = observer(function LoginPageV2Container() {
	const { t } = useLingui();
	...
	switch (loginState) {
		case 'default':
			return <LoginPageV2Content />;
		case 'mfa':
			return <LoginPageV2MFA />;
		default:
			return null;
	}
});

export default LoginPageV2Container;
