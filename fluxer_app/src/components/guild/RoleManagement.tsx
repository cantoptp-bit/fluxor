...
const ManageRolesMenuContent: React.FC<ManageRolesMenuContentProps> = observer(function ManageRolesMenuContent({
	...
	if (allRoles.length === 0) {
		const noRolesLabel =
			guild?.guildType === 'server'
				? t`No roles yet, add roles in Server Settings`
				: t`No roles yet, add roles in Community Settings`;
		return (
			<MenuGroup>
...
export const ManageRolesBottomSheet: React.FC<ManageRolesBottomSheetProps> = observer(function ManageRolesBottomSheet({
	...
	}, [allRoles, currentMember, guild, guildId, userId, handleOpenGuildSettings]);
...
function openNoRolesModal(guildId: string) {
	function NoRolesModalDescription() {
		...
		const settingsLinkLabel =
			guild?.guildType === 'server'
				? t`Server Settings > Roles & Permissions`
		...
		return guild?.guildType === 'server' ? (
			<Trans>
				There are no roles to assign in this server at this time, but you can create a new role in{' '}
