...
const LazyMemberList = observer(function LazyMemberList({guild, channel}: LazyMemberListProps) {
	...
	const apiFetchAttemptedRef = useRef<Set<string>>(new Set());
	...
	const storeMembers = GuildMemberStore.getMembers(guild.id);
	const gatewayHasList = Boolean(memberListState && memberListState.items.size > 0);
	const hasFallbackMembers = storeMembers.length > 0;
	const isLoading = !gatewayHasList && !hasFallbackMembers;
	...
	fetch('http://127.0.0.1:7242/ingest/3edd0748-1198-484b-9297-600217c1c572', {
		...
		body: JSON.stringify({
			location: 'ChannelMembers.tsx:LazyMemberList',
			message: 'LazyMemberList state',
			data: {
				guildId: guild.id,
				gatewayHasList,
				hasFallbackMembers,
				storeMembersLength: storeMembers.length,
				isLoading,
	...
	// When gateway never sends member list (e.g. Node gateway), fetch members from REST API once per guild
	useEffect(() => {
		// #region agent log
		if (memberListUpdatesDisabled || gatewayHasList || hasFallbackMembers) {
			fetch('http://127.0.0.1:7242/ingest/3edd0748-1198-484b-9297-600217c1c572', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					location: 'ChannelMembers.tsx:useEffect-skip',
					message: 'fetch effect skipped',
					data: { guildId: guild.id, reason: 'guard', memberListUpdatesDisabled, gatewayHasList, hasFallbackMembers },
					timestamp: Date.now(),
					hypothesisId: 'H1',
				}),
			}).catch(() => {});
		...
		if (apiFetchAttemptedRef.current.has(guild.id)) {
			fetch('http://127.0.0.1:7242/ingest/3edd0748-1198-484b-9297-600217c1c572', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					location: 'ChannelMembers.tsx:useEffect-skip',
					message: 'fetch effect skipped (already attempted)',
		...
		// #endregion
		apiFetchAttemptedRef.current.add(guild.id);
		fetch('http://127.0.0.1:7242/ingest/3edd0748-1198-484b-9297-600217c1c572', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				location: 'ChannelMembers.tsx:useEffect-fetch',
				message: 'calling fetchMembersFromApi',
				data: { guildId: guild.id },
				timestamp: Date.now(),
				hypothesisId: 'H1',
			}),
		}).catch(() => {});
		void GuildMemberStore.fetchMembersFromApi(guild.id, {limit: 100}).then((members) => {
			// Only keep "attempted" if we got members; otherwise allow retry (e.g. after refresh)
			if (members.length === 0) {
				apiFetchAttemptedRef.current.delete(guild.id);
			}
		});
	}, [guild.id, memberListUpdatesDisabled, gatewayHasList, hasFallbackMembers]);
	...
	// Fallback: show members from store when gateway never sent a member list (e.g. Node gateway)
	if (hasFallbackMembers && !gatewayHasList) {
		const memberGroups = MemberListUtils.getMemberGroups(guild, channel);
		// #region agent log
		fetch('http://127.0.0.1:7242/ingest/3edd0748-1198-484b-9297-600217c1c572', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				location: 'ChannelMembers.tsx:fallback-render',
				message: 'rendering fallback member list',
				data: { guildId: guild.id, groupCount: memberGroups.length, storeMembersLength: storeMembers.length },
				timestamp: Date.now(),
				hypothesisId: 'H4_H5',
			}),
		}).catch(() => {});
			...
			<MemberListContainer channelId={channel.id}>
				{memberGroups.map((group) => (
					<LazyMemberListGroup
						key={group.id}
						guild={guild}
						group={{id: group.id, count: group.count}}
