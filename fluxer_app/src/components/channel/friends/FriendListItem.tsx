...
export const FriendListItem: React.FC<FriendListItemProps> = observer((props) => {
		...
		async (e: React.MouseEvent) => {
			...
			try {
				...
				ToastActionCreators.createToast({
					type: 'error',
					children: message ?? (t`Could not open message. Try again.` as string),
				});
			}
		},
		[userId, t],
	...
	const customStatus = usePresenceCustomStatus({
		userId,
		enabled: relationshipType === RelationshipTypes.FRIEND && Boolean(user),
	});

	// Fetch user if missing (e.g. relationship loaded without user partial) so the row is not hidden
	useEffect(() => {
		if (!user) {
			void UserProfileActionCreators.fetch(userId).catch(() => {
				// Ignore fetch errors; row will show placeholder
			});
		}
	}, [userId, user]);

	if (!user) {
		// Show placeholder so friend is not hidden; fetch in progress will populate UserStore
		return (
			<FocusRing>
				<div
					ref={itemRef}
					...
					onKeyDown={(e) => {
						if (e.key === 'Enter' || e.key === ' ') openProfile(userId);
					}}
				>
					<div className={styles.friendInfo}>
						<div className={styles.avatarPlaceholder} />
						<div className={styles.friendDetails}>
							<span className={styles.friendName}>{userId}</span>
							<span className={styles.friendSubtext}>
								<Trans>Loading…</Trans>
							</span>
						</div>
					</div>
