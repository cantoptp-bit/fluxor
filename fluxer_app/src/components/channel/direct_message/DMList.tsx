...
const getDmRouteChannelId = (pathname: string): string | null => {
	...
	const segmentAfterMe = segments[3];
	// /channels/@me/temp/:tempChatId is temp chat, not a DM channel
	if (segmentAfterMe === 'temp') return null;
	return segmentAfterMe ?? null;
};

const getCurrentTempChatId = (pathname: string): string | null => {
	if (!Routes.isTempChatRoute(pathname)) return null;
	const segments = pathname.split('/');
...
export const DMList = observer(() => {
	...
	const [tempChats, setTempChats] = useState<Array<TempChatApi.TempChatSummary>>([]);
	const [dmListMode, setDmListMode] = useState<'direct' | 'temp'>('direct');
	...
	const currentTempChatId = getCurrentTempChatId(location.pathname);
	...
	useEffect(() => {
		...
		TempChatApi.listTempChats()
			.then(setTempChats)
			.catch(() => setTempChats([]));
	}, [currentUserId]);

	// Sync mode from route so the correct tab is selected when navigating
	useEffect(() => {
		if (currentTempChatId) setDmListMode('temp');
		else if (routeDmChannelId) setDmListMode('direct');
	}, [currentTempChatId, routeDmChannelId]);
	...
	const navigateTo = (path: string) => () => {
		if (Routes.isDMRoute(path)) {
			...
			} else if (Routes.isTempChatRoute(path)) {
				// Temp chat URL: don't treat last segment as DM channel id
				NavigationActionCreators.selectChannel(ME, null);
			} else {
				const segments = path.split('/');
				const segmentAfterMe = segments[3];
				if (segmentAfterMe && segmentAfterMe !== ME) {
					NavigationActionCreators.selectChannel(ME, segmentAfterMe);
				} else {
					NavigationActionCreators.selectChannel(ME, null);
				}
			}
	...
	const navigateToTempChat = (tempChatId: string) => () => {
		RouterUtils.transitionTo(Routes.tempChat(tempChatId));
		NavigationActionCreators.selectChannel(ME, null);
		if (MobileLayoutStore.isMobileLayout()) {
			LayoutActionCreators.updateMobileLayoutState(false, true);
		}
			...
			<div className={styles.mobileContainer}>
				...
				<Scroller className={styles.mobileScroller} key="dm-list-mobile-scroller">
					<div className={styles.mobileScrollerContent}>
						...
						<div className={styles.dmListModeToggle} role="tablist" aria-label={t`Switch between direct messages and temp chats`}>
							<FocusRing offset={-2}>
								<button
									type="button"
									role="tab"
									aria-selected={dmListMode === 'direct'}
									className={dmListMode === 'direct' ? styles.dmListModeButtonActive : styles.dmListModeButton}
									onClick={() => setDmListMode('direct')}
								>
							...
							<FocusRing offset={-2}>
								<button
									type="button"
									role="tab"
									aria-selected={dmListMode === 'temp'}
									className={dmListMode === 'temp' ? styles.dmListModeButtonActive : styles.dmListModeButton}
									onClick={() => setDmListMode('temp')}
								>
									<Trans>Temp chats</Trans>
								</button>
							</FocusRing>
						</div>
						...
						{dmListMode === 'temp' && (
							tempChats.length === 0 ? (
								<div className={styles.dmListModeEmpty}>
									<Trans>No temporary chats yet. Start one from a friend's profile.</Trans>
								</div>
							) : (
								tempChats.map((chat) => {
									const otherId =
										chat.participant_ids[0] === currentUserId
									...
									return (
										<FocusRing key={chat.id} offset={-2}>
											<button
												type="button"
												onClick={navigateToTempChat(chat.id)}
												className={
													isSelected ? styles.dmItemMobileSelected : styles.dmItemMobile
												}
											>
												<div className={styles.dmItemContent}>
													<div className={styles.dmItemAvatarWrapper}>
													...
													<div className={styles.dmItemInfo}>
														...
														<span className={styles.dmItemSubtext}>
											...
											<FocusRing key={chat.id} offset={-2}>
												...
												>
													<div className={styles.clickableItemInner}>
														...
														<LockIcon weight="fill" className={styles.tempChatListLockIcon} />
													</div>
												</button>
											</FocusRing>
										);
