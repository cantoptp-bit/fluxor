...
export const DMWelcomeSection: React.FC<DMWelcomeSectionProps> = observer(function DMWelcomeSection({userId, channel}) {
	...
	// When relationship is unknown (e.g. opened DM before relationships loaded), fetch so we show correct action and enable temp chat
	useEffect(() => {
		if (!user?.id || user.bot || relationshipType !== undefined) return;
		if (relationshipsFetched) return;
		RelationshipActionCreators.fetchRelationships()
			.then(() => setRelationshipsFetched(true))
			.catch((err) => {
				logger.error('Failed to fetch relationships for DM welcome', err);
				setRelationshipsFetched(true);
			});
	}, [user?.id, user?.bot, relationshipType, relationshipsFetched]);
	...
	const shouldShowActionButton =
		...
		(relationshipType === RelationshipTypes.INCOMING_REQUEST ||
			...
			(relationshipType === undefined && relationshipsFetched));
	...
	const renderActionButton = () => {
		...
		switch (relationshipType) {
			case undefined: {
				// Not loaded yet: show nothing so we don't wrongly show "Send Friend Request" for a friend
				if (!relationshipsFetched) return null;
