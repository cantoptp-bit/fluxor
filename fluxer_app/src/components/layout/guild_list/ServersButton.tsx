...
import {CaretDownIcon, CirclesThreeIcon} from '@phosphor-icons/react';
import {clsx} from 'clsx';
import {motion} from 'framer-motion';
import {observer} from 'mobx-react-lite';
import {useRef} from 'react';

/** Section header for the Servers list in the left sidebar. Click to expand/collapse the list below. */
interface ServersButtonProps {
	className?: string;
	expanded?: boolean;
	onToggle?: () => void;
	itemCount?: number;
...
export const ServersButton = observer(
	({className, expanded = true, onToggle, itemCount = 0}: ServersButtonProps = {}) => {
		const {t} = useLingui();
			...
			<>
				...
				<div className={styles.guildListSectionHeaderInner}>
					<div className={styles.relative}>
						...
						>
							<CirclesThreeIcon weight="fill" className={styles.favoritesIcon} />
						</motion.div>
					</div>
					{isClickable && (
						<motion.span
