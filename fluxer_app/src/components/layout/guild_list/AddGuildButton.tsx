...
import {HouseIcon, LinkIcon, PlusIcon, UsersThreeIcon} from '@phosphor-icons/react';
		...
		ContextMenuActionCreators.openFromEvent(e, ({onClose}) => (
			<MenuGroup>
				<MenuItem
					...
					onClick={() => {
						handleAddGuild('create_server');
						onClose();
					}}
				>
					<Trans>Create Server</Trans>
				</MenuItem>
				<MenuItem
					icon={<UsersThreeIcon className={styles.menuIcon} />}
					onClick={() => {
	...
	return (
		<div className={clsx(guildStyles.addGuildButton, contextMenuOpen && guildStyles.contextMenuHover)}>
			<Tooltip
				position="right"
				size="large"
				text={() => <TooltipWithKeybind label={t`Add a Server or Community`} action="create_or_join_server" />}
			>
				<FocusRing offset={-2} focusTarget={buttonRef} ringTarget={iconRef}>
					<button
						type="button"
						aria-label={t`Add a Server or Community`}
