...
const InvitesDisabledNagbar = observer(({isMobile, guildId}: {isMobile: boolean; guildId: string}) => {
	...
	const handleEnableInvites = () => {
		...
		ModalActionCreators.push(
			modal(() => (
				<ConfirmModal
					title={isServer ? t`Enable invites for this server` : t`Enable invites for this community`}
					description={
						isServer ? (
							<Trans>
								Are you sure you want to enable invites? This will allow users to join this server through invite links
								again.
							</Trans>
						) : (
							<Trans>
...
export const GuildLayout = observer(({children}: {children: React.ReactNode}) => {
	...
	const channelFetchAttemptedRef = useRef<Set<string>>(new Set());
	useEffect(() => {
		if (!guildId || !guild || guildUnavailable || guildNotFound) return;
		const guildChannels = ChannelStore.getGuildChannels(guildId);
		if (guildChannels.length > 0) return;
		if (channelFetchAttemptedRef.current.has(guildId)) return;
		channelFetchAttemptedRef.current.add(guildId);
		void ChannelStore.fetchGuildChannelsFromApi(guildId);
	}, [guildId, guild, guildUnavailable, guildNotFound]);
