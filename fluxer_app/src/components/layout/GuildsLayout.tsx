...
import {CommunityButton} from '@app/components/layout/guild_list/CommunityButton';
...
import {ServersButton} from '@app/components/layout/guild_list/ServersButton';
...
import type {GuildDragItem, GuildDropResult} from '@app/components/layout/types/DndTypes';
...
const GuildList = observer(() => {
	...
	const fullOrganizedItems = GuildListStore.getOrganizedGuildList();
	const serverItems = GuildListStore.getOrganizedGuildListByType('server');
	const communityItems = GuildListStore.getOrganizedGuildListByType('community');
	...
	const handleGuildDrop = useCallback(
		(item: GuildDragItem, result: GuildDropResult) => {
			...
			const oldIndex = fullOrganizedItems.findIndex((i) => getOrganizedItemKey(i) === sourceKey);
			const targetIndex = fullOrganizedItems.findIndex((i) => getOrganizedItemKey(i) === targetKey);
			if (oldIndex === -1 || targetIndex === -1) return;

			let newIndex = position === 'after' ? targetIndex + 1 : targetIndex;
			if (oldIndex < targetIndex && position === 'after') newIndex--;

			const newOrganizedItems = [...fullOrganizedItems];
	...
	const handleDragStateChange = useCallback((_item: GuildDragItem | null) => {
		// No-op: drag is disabled in Servers/Communities sections.
	}, []);
	...
	return (
		<div className={styles.guildListScrollerWrapper}>
			...
			<div className={styles.guildListBottomSection}>
				<DiscoveryButton />
				<AddGuildButton />
