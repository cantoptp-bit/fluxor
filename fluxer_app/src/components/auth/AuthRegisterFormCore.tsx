...
export function AuthRegisterFormCore({
	...
	const handleRegisterSubmit = async (values: Record<string, string>) => {
		...
		const validDateOfBirth = /^\d{4}-\d{2}-\d{2}$/.test(dateOfBirth);
		if (!validDateOfBirth) {
			form.setError('date_of_birth', t`Please enter a valid date of birth`);
			return;
		}
	...
	return (
		<form className={styles.form} onSubmit={form.handleSubmit}>
			...
			<DateOfBirthField
				...
				error={form.getError('date_of_birth') || fieldErrors?.date_of_birth}
			/>
