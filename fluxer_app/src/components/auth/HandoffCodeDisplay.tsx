...
interface HandoffCodeDisplayProps {
	...
	error: string | null;
	onRetry?: () => void;
	/** When provided, show a "Continue in this browser" button so the user can complete sign-in here instead of pasting the code in the desktop app. */
	onContinueInBrowser?: () => Promise<void>;
}
		...
		<div className={styles.container}>
			...
			{onContinueInBrowser && (
				<p className={styles.continueHint}>
					<Trans>Using a different device? Paste the code there. Or sign in here:</Trans>
				</p>
			)}
			{onContinueInBrowser && (
				<Button
					...
					fitContainer
				>
					<Trans>Continue in this browser</Trans>
				</Button>
			)}
		</div>
	);
