...
export const InviteAcceptModal = observer(function InviteAcceptModal({code}: InviteAcceptModalProps) {
	...
	const guildFromInvite = invite && isGuildInvite(invite) ? invite.guild : null;
	const isServer = (guildFromInvite as {guild_type?: string})?.guild_type === 'server';
	...
	const primaryLabel = useMemo(() => {
		...
		switch (primaryActionType) {
			...
			case GuildInvitePrimaryAction.GoToCommunity:
				return isServer ? t`Go to Server` : t`Go to Community`;
			default:
				return isServer ? t`Join Server` : t`Join Community`;
		}
	}, [isGroupDM, primaryActionType, isServer]);
