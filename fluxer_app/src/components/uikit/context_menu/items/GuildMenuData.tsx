...
export function useGuildMenuData(guild: GuildRecord, options: UseGuildMenuDataOptions): GuildMenuData {
	...
	const groups = useMemo(() => {
		...
		if (canAccessGuildSettings) {
			...
			const settingsLabel = guild.guildType === 'server' ? t`Server Settings` : t`Community Settings`;
			quickActions.push({
		...
		const settingsItems: Array<MenuItemType> = [
			...
			},
			{
				...
				label: guild.guildType === 'server' ? t`Edit Server Profile` : t`Edit Community Profile`,
		...
		if (!isOwner) {
			const leaveLabel = guild.guildType === 'server' ? t`Leave Server` : t`Leave Community`;
			const reportLabel = guild.guildType === 'server' ? t`Report Server` : t`Report Community`;
