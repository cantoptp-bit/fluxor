...
export class MultiAccountGatewaySocket extends EventEmitter<MultiAccountGatewaySocketEvents> {
	...
	private handleRelayOpen = (): void => {
		logger.debug('Relay WebSocket connection established');
	...
	private scheduleRelayReconnect(): void {
		...
		logger.debug(`Scheduling relay reconnect in ${delay}ms`);
	...
	disconnectRelay(code = 1000, reason = 'Client disconnecting'): void {
		logger.debug(`Relay disconnect requested: [${code}] ${reason}`);
	...
	async addAccount(
		...
		logger.debug('Added account:', accountId, 'for instance:', instanceDomain);
	...
	handleNetworkStatusChange(online: boolean): void {
		logger.debug(`Network status: ${online ? 'online' : 'offline'}`);
	...
	resetAccount(accountId: string, shouldReconnect = true): void {
		...
		logger.debug(`Resetting account: ${accountId} (reconnect=${shouldReconnect})`);
	...
	simulateNetworkDisconnect(accountId: string): void {
		...
		logger.debug('Simulating network disconnect for account:', accountId);
	...
	private handleAccountPayload(accountId: string, payload: GatewayPayload): void {
		...
		switch (payload.op) {
			...
			case GatewayOpcodes.RECONNECT:
				logger.debug('Server requested reconnect for account:', accountId);
	...
	private handleAccountDispatch(accountId: string, payload: GatewayPayload): void {
		...
		switch (payload.t) {
			case 'READY': {
				...
				logger.debug(`Account READY: ${accountId}, session=${account.sessionId}`);
			...
			case 'RESUMED':
				this.clearHelloTimeout(accountId);
				this.updateAccountState(accountId, AccountConnectionState.Connected);
				logger.debug('Account session resumed:', accountId);
	...
	private handleAccountInvalidSession(accountId: string, payload: GatewayPayload): void {
		...
		logger.debug(`Account session invalidated: ${accountId} (resumable=${isResumable})`);
	...
	private sendAccountIdentify(accountId: string): void {
		...
		logger.debug('Sending IDENTIFY for account:', accountId);
	...
	private sendAccountResume(accountId: string): void {
		...
		logger.debug(`Sending RESUME for account: ${accountId}, session=${account.sessionId}`);
	...
	private updateRelayState(nextState: RelayConnectionState): void {
		...
		logger.debug(`Relay state: ${previous} -> ${nextState}`);
	...
	private updateAccountState(accountId: string, nextState: AccountConnectionState): void {
		...
		logger.debug(`Account ${accountId} state: ${previous} -> ${nextState}`);
	...
	destroy(): void {
		...
		logger.debug('MultiAccountGatewaySocket destroyed');
	}
}
