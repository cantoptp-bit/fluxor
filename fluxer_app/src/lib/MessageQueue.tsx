...
function isMustStartSessionError(error: HttpError): boolean {
	if (error?.status !== 400) return false;
	const body = getApiErrorBody(error);
	if (body?.code !== APIErrorCodes.INVALID_FORM_BODY || !Array.isArray(body.errors)) return false;
	return body.errors.some((e) => e.code === ValidationErrorCodes.MUST_START_SESSION_BEFORE_SENDING);
}
...
class MessageQueue extends Queue<MessageQueuePayload, HttpResponse<Message> | undefined> {
	...
	private handleSendError(
		...
		if (isMustStartSessionError(error)) {
			const systemMessage = createSystemMessage(
				channelId,
				i18n._(msg`You must start a session before sending messages.`),
			);
			MessageActionCreators.createOptimistic(channelId, systemMessage.toJSON());
			return;
		}
