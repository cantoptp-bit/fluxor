...
import { createNodeGatewayServer, type NodeGatewayServer } from '@app/utils/NodeGatewayServer';
...
export async function createFluxerServer(options: FluxerServerOptions = {}): Promise<FluxerServerResult> {
	...
	let nodeGateway: NodeGatewayServer | null = null;
	...
	const start = async (): Promise<void> => {
		...
		// Only try the Erlang binary in production or when test_mode explicitly enables it.
		const shouldStartErlangGateway =
			config.services.gateway && (config.env === 'production' || config.dev.test_mode_enabled);
		if (shouldStartErlangGateway) {
			Logger.info('Initializing Gateway Process Manager');
			gatewayManager = createGatewayProcessManager();
			try {
				await gatewayManager.start();
			} catch (err) {
				Logger.warn({ err }, 'Failed to start Erlang gateway process manager');
				// Fall through: Node gateway fallback below.
			}
		}

		// Always start the built-in Node gateway when the Erlang gateway is not running.
		// This includes normal development mode where test_mode_enabled is false (the default).
		const erlangRunning = shouldStartErlangGateway && gatewayManager?.isRunning();
		if (config.services.gateway && !erlangRunning) {
			if (!nodeGateway) {
				Logger.info('Starting built-in Node gateway (Erlang gateway not available or not configured for this env)');
				nodeGateway = createNodeGatewayServer();
				nodeGateway.start();
			}
		}
		...
		if (mounted.services.jsConnectionManager) {
			...
			try {
				...
				const isLiteMode = process.env.FLUXER_LITE_MODE === '1';
				if (isLiteMode) {
					workerLogger.warn(
						{ error: error instanceof Error ? error.message : 'Unknown error' },
						'Background worker initialization failed (Lite Mode active - continuing)',
					);
				} else {
					throw error;
				}
					...
					if (server !== null) {
						...
						await new Promise<void>((resolve) => {
							...
							server!.close((err) => {
								...
								if (nodeGateway) {
									Logger.info('Stopping Node gateway');
									nodeGateway.stop();
								}
