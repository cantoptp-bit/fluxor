...
const GATEWAY_PATH_DEFAULT = '/opt/fluxer_gateway/bin/fluxer_gateway';
const GATEWAY_DIST_PORT = 9100;

function getGatewayPath(): string {
	return process.env['FLUXER_GATEWAY_BINARY_PATH'] ?? GATEWAY_PATH_DEFAULT;
}
...
export function createGatewayProcessManager(): GatewayProcessManager {
	...
	const start = async (): Promise<void> => {
		...
		if (!existsSync(gatewayPath)) {
			logger.warn(
				{ path: gatewayPath },
				'Gateway binary not found, WebSocket gateway unavailable. Set FLUXER_GATEWAY_BINARY_PATH to a local build (e.g. fluxer_gateway/_build/dev/rel/fluxer_gateway/bin/fluxer_gateway) or install at /opt/fluxer_gateway.',
			);
			startupFailed = true;
			return;
		}
		...
		try {
			gatewayProcess = spawn(gatewayPath, ['foreground'], {
				env: gatewayEnv,
