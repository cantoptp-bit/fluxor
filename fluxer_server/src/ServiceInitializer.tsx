...
import { DevSeedService } from '@fluxer/api/src/dev/DevSeedService';
...
import { MockKVProvider } from '@fluxer/kv_client/src/MockKVProvider';
...
function createKVProvider(config: Config): IKVProvider {
	if (process.env.FLUXER_LITE_MODE === '1') {
		return new MockKVProvider();
	}
...
export async function initializeAllServices(context: ServiceInitializationContext): Promise<{
	...
	try {
		...
		if (context.config.env === 'development') {
			rootLogger.info('Initializing Dev Seed service');
			const devSeed = new DevSeedService();
			initializers.push({
				name: 'Dev Seed',
				initialize: async () => {
					await devSeed.seed();
		...
		try {
			...
			if (isLiteMode) {
				rootLogger.warn(
					...
					'Service initialization failed (Lite Mode active - continuing)',
				);
				continue;
			}
