...
## Site not loading or blank page

If the Vercel site shows a blank page or never finishes loading:

1. **Root Directory must be the repository root**  
   In Vercel **Settings → General**, set **Root Directory** to `.` or leave empty. The build and `api/` folder only work when run from the repo root.

2. **Redeploy after changes**  
   After adding or changing `vercel.json` or `api/fluxer-config.js`, trigger a full redeploy (Deployments → … → Redeploy) so the serverless function and rewrites are applied.

3. **Check the build**  
   In the Vercel deployment log, confirm `build:vercel` completes and `fluxer_app/dist` contains `index.html` and an `assets/` folder. If the build fails (e.g. missing `config/config.vercel.json`), fix the error and redeploy.

4. **Optional: set FLUXER_PUBLIC_DOMAIN**  
   If you want the app to connect to a backend, set the env var (see above). The app will still load and show the login page without it; you may see a “backend unreachable” notice until the backend URL is set.
