	...
	return (
		<section class="mx-auto w-full max-w-5xl">
			<div class="mb-6">
				<a
					href={href(ctx, '/help')}
					class="inline-flex items-center gap-1 text-muted-foreground text-sm underline-offset-2 transition-colors hover:text-foreground focus:underline focus:outline-none"
				>
			...
			<header class="mb-10 space-y-3">
				...
				<h1 class="font-bold text-4xl text-foreground tracking-tight">{article.title}</h1>
				{article.description ? <p class="max-w-2xl text-lg text-muted-foreground">{article.description}</p> : null}
			...
			<div class="grid gap-10 lg:grid-cols-[minmax(0,1fr)_220px]">
				<div id="help-content" class="policy-prose min-w-0" dangerouslySetInnerHTML={{__html: renderedContent}} />
				<aside id="help-toc" class="hidden lg:sticky lg:top-24 lg:block lg:self-start">
...
function renderToc(title: string, headings: ReadonlyArray<HeadingEntry>): JSX.Element | null {
	...
	return (
		<nav class="space-y-2 rounded-xl border border-gray-200 bg-gray-50/50 p-4" aria-label={title}>
			<h2 class="mb-3 font-semibold text-foreground text-sm">{title}</h2>
				...
				{filtered.map((heading) => (
					<li style={`margin-left: ${(heading.level - minLevel) * 12}px`}>
						<a
							...
							class="block py-1 underline-offset-2 transition-colors hover:text-foreground focus:underline focus:outline-none"
						>
							{heading.title}
						</a>
					</li>
