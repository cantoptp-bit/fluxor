...
function renderHelpIndexBody(ctx: MarketingContext, articles: ReadonlyArray<HelpArticleMetadata>): JSX.Element {
	return (
		<section class="mx-auto w-full max-w-5xl">
			...
			<div class="mb-10">
				<label for="help-search" class="sr-only">
					{ctx.i18n.getMessage('company_and_resources.help.search_placeholder', ctx.locale)}
				</label>
				<input
					id="help-search"
					type="search"
					...
					autocomplete="off"
					aria-label={ctx.i18n.getMessage('company_and_resources.help.search_placeholder', ctx.locale)}
				/>
			</div>
...
function renderHelpCard(ctx: MarketingContext, article: HelpArticleMetadata): JSX.Element {
	return (
		<a
			...
			class="help-card block text-left no-underline"
			data-help-card=""
		...
		>
			...
			<h2 class="mb-1 font-semibold text-foreground text-lg">{article.title}</h2>
			<p class="text-muted-foreground text-sm leading-relaxed">{article.description}</p>
		</a>
	);
}
