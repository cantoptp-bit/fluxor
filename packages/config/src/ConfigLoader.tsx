...
export async function loadConfig(configPaths: Array<string> = DEFAULT_CONFIG_PATHS): Promise<MasterConfig> {
	...
	// Optional env overrides for public URL when behind HTTPS proxy (e.g. Cloudflare Tunnel)
	const publicScheme = process.env['FLUXER_PUBLIC_SCHEME'];
	const publicPortRaw = process.env['FLUXER_PUBLIC_PORT'];
	const domain = (merged['domain'] ?? {}) as Record<string, unknown>;
	if (publicScheme === 'https' || publicScheme === 'http') {
		merged['domain'] = {...domain, public_scheme: publicScheme};
	}
	if (publicPortRaw !== undefined && publicPortRaw !== '') {
		const publicPort = Number.parseInt(publicPortRaw, 10);
		if (!Number.isNaN(publicPort) && publicPort > 0) {
			merged['domain'] = {...(merged['domain'] as Record<string, unknown>), public_port: publicPort};
		}
	}
