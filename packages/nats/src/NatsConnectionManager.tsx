...
export class NatsConnectionManager implements INatsConnectionManager {
	...
	async connect(): Promise<void> {
		...
		if (process.env.FLUXER_LITE_MODE === '1') {
			console.error(`[nats-manager] [PID:${process.pid}] Starting in LITE_MODE - using MockNatsConnection`);
			const { MockNatsConnection } = await import('./MockNatsConnection');
			this.connection = new MockNatsConnection() as any as NatsConnection;
			return;
		}
