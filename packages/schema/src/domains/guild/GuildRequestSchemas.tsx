...
export const GuildTypeSchema = z.enum(['server', 'community']).describe('Whether the guild is a server or a community');
export type GuildType = z.infer<typeof GuildTypeSchema>;

export const GuildCreateRequest = z
	.object({
		...
		guild_type: GuildTypeSchema.optional().default('server').describe('Whether to create a server or a community'),
		list_in_discovery: z
			.boolean()
			.optional()
			.describe('Whether to submit this community for the discovery list (community only)'),
		discovery_description: z
			.string()
			.min(10, 'Discovery description must be at least 10 characters')
			.max(300, 'Discovery description must be at most 300 characters')
			.optional()
			.describe('Short description for the community explorer (required when list_in_discovery is true)'),
		discovery_category: z
			.number()
			.int()
			.min(0)
			.max(8)
			.optional()
			.describe('Discovery category (0-8, required when list_in_discovery is true)'),
	})
	.refine(
		(data) => {
			if (data.guild_type !== 'community' || !data.list_in_discovery) return true;
			return (
				typeof data.discovery_description === 'string' &&
				data.discovery_description.length >= 10 &&
				data.discovery_description.length <= 300 &&
				typeof data.discovery_category === 'number' &&
				data.discovery_category >= 0 &&
				data.discovery_category <= 8
			);
		},
		{ message: 'When listing in discovery, description (10-300 chars) and category (0-8) are required', path: ['discovery_description'] },
	);
