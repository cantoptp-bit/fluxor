...
describe('GuildResponse', () => {
	...
	it('accepts guild_type server', () => {
		const guild = {...validGuild, guild_type: 'server'};
		const result = GuildResponse.safeParse(guild);
		expect(result.success).toBe(true);
		if (result.success) expect(result.data.guild_type).toBe('server');
	});

	it('accepts guild_type community', () => {
		const guild = {...validGuild, guild_type: 'community'};
		const result = GuildResponse.safeParse(guild);
		expect(result.success).toBe(true);
		if (result.success) expect(result.data.guild_type).toBe('community');
	});

	it('defaults guild_type to server when omitted', () => {
		const result = GuildResponse.safeParse(validGuild);
		expect(result.success).toBe(true);
		if (result.success) expect(result.data.guild_type).toBe('server');
	});

	it('rejects invalid guild_type', () => {
		const guild = {...validGuild, guild_type: 'invalid'};
		const result = GuildResponse.safeParse(guild);
		expect(result.success).toBe(false);
	});
});
