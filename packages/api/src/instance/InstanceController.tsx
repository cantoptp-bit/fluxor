			...
			try {
				...
				try {
					...
					if (limitConfigService) {
						limits = limitConfigService.getConfigWireFormat();
					}
				} catch (err) {
					// Fallback
				}
				...
				try {
					...
					if (instanceConfigRepository) {
						instanceConfig = await instanceConfigRepository.getInstanceConfig();
					}
				} catch (err) {
					// Fallback
				}
				...
				try {
					...
					if (ssoService) {
						const status = await ssoService.getPublicStatus();
						if (status) {
							sso = status;
						}
					}
				...
				const response: Record<string, unknown> = {
					...
					features: {
						sms_mfa_enabled: Config.dev?.testModeEnabled || Config.sms?.enabled || false,
						voice_enabled: Config.voice?.enabled || false,
						stripe_enabled: Config.stripe?.enabled || false,
						self_hosted: Config.instance?.selfHosted || false,
						manual_review_enabled: instanceConfig?.manualReviewEnabled ?? false,
					},
					gif: {
						provider: Config.gif?.provider ?? 'tenor',
					},
					sso,
					limits,
					push: {
				...
				if (Config.federation?.enabled) {
					try {
						...
						const instanceDomain = Config.domain?.baseDomain ?? 'localhost';
				...
				console.error('[CRITICAL] Discovery handler failed completely', generalError);
				// Absolute emergency fallback that should pass Zod validation
				return ctx.json({
					api_code_version: API_CODE_VERSION,
					endpoints: {
						api: '', api_client: '', api_public: '', gateway: '', media: '', static_cdn: '',
						marketing: '', admin: '', invite: '', gift: '', webapp: ''
					},
					captcha: { provider: 'none', hcaptcha_site_key: null, turnstile_site_key: null },
					features: { sms_mfa_enabled: false, voice_enabled: false, stripe_enabled: false, self_hosted: true, manual_review_enabled: false },
					gif: { provider: 'tenor' },
					sso: { enabled: false, enforced: false, display_name: null, redirect_uri: '' },
					limits: { version: 2, traitDefinitions: [], rules: [], defaultsHash: 'default' },
