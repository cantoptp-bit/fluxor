...
import {TempChatMessageRepository} from '@fluxer/api/src/user/repositories/TempChatMessageRepository';
import {TempChatRepository} from '@fluxer/api/src/user/repositories/TempChatRepository';
import {UserContactChangeLogRepository} from '@fluxer/api/src/user/repositories/UserContactChangeLogRepository';
import {UserE2EKeyRepository} from '@fluxer/api/src/user/repositories/UserE2EKeyRepository';
...
import {TempChatRequestService} from '@fluxer/api/src/user/services/TempChatRequestService';
import {TempChatService} from '@fluxer/api/src/user/services/TempChatService';
import {UserE2EKeyService} from '@fluxer/api/src/user/services/UserE2EKeyService';
...
export const ServiceMiddleware = createMiddleware<HonoEnv>(async (ctx, next) => {
	...
	const userE2EKeyRepository = new UserE2EKeyRepository();
	const tempChatRepository = new TempChatRepository();
	const tempChatMessageRepository = new TempChatMessageRepository();
	const userE2EKeyService = new UserE2EKeyService(userE2EKeyRepository);
	const tempChatService = new TempChatService(
		userRepository,
		tempChatRepository,
		tempChatMessageRepository,
		snowflakeService,
	);
	const tempChatRequestService = new TempChatRequestService(
		tempChatService,
		userE2EKeyService,
		userRepository,
	);
	...
	ctx.set('tempChatRequestService', tempChatRequestService);
