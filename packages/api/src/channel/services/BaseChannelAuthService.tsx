...
export abstract class BaseChannelAuthService {
	...
	async getChannelAuthenticated({
		...
		if (channelId === (0n as ChannelID)) {
			channelId = userIdToChannelId(userId);
		}

		if (this.isPersonalNotesChannel({ userId, channelId })) {
			let channel = await this.channelRepository.channelData.findUnique(channelId);
			if (!channel) {
				// Seed missing personal notes channel on the fly
				channel = await this.channelRepository.channelData.upsert({
	...
	isPersonalNotesChannel({ userId, channelId }: { userId: UserID; channelId: ChannelID }): boolean {
		return channelId === (0n as ChannelID) || userIdToChannelId(userId) === channelId;
	}
