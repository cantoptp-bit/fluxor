...
function codeOrUndefined(error: Error): number | undefined {
...
export class NatsGatewayRpcTransport implements IGatewayRpcTransport {
	...
	async call(method: string, params: Record<string, unknown>): Promise<unknown> {
		...
		const payload = this.codec.encode(JSON.stringify(params, bigintReplacer));
		...
		try {
			...
			if (error instanceof Error) {
				if (
					error.message === 'NO_RESPONDERS' ||
					error.name === 'NoRespondersError' ||
					error.message === '503' ||
					(codeOrUndefined(error) === 503)
				) {
