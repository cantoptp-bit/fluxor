...
class DisabledBlueskyOAuthService implements IBlueskyOAuthService {
	...
	async authorize(): Promise<BlueskyAuthorizeResult> {
		throw new Error('Bluesky OAuth is disabled in Lite Mode');
	}
	async callback(): Promise<BlueskyCallbackResult> {
		throw new Error('Bluesky OAuth is disabled in Lite Mode');
	}
	async restoreAndVerify(): Promise<{ handle: string } | null> {
		return null;
	}
...
export class BlueskyOAuthService implements IBlueskyOAuthService {
	...
	static async create(
		...
		try {
			...
			const isDev = process.env.NODE_ENV === 'development' || process.env.FLUXER_LITE_MODE === '1';
			if (isDev) {
				Logger.warn(
					{ error: error instanceof Error ? error.message : String(error) },
					'Failed to initialize Bluesky OAuth service, using disabled fallback in development mode',
				);
				return new DisabledBlueskyOAuthService();
			}
			throw error;
		}
	}
