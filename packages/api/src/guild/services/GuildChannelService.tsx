...
export class GuildChannelService {
	...
	async getChannels(params: {
		...
		let viewableChannelIds: Array<ChannelID>;
		try {
			viewableChannelIds = await this.gatewayService.getViewableChannels({
				guildId: params.guildId,
				userId: params.userId,
			});
		} catch (error) {
			// When gateway is unavailable (e.g. built-in Node gateway without this RPC), treat all guild channels as viewable
			if (error instanceof ServiceUnavailableError || error instanceof GatewayTimeoutError) {
				viewableChannelIds = channels.map((c) => c.id);
			} else {
				throw error;
			}
		}
