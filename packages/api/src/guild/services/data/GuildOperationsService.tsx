...
import { mapUserSettingsToResponse, mapUserToPartialResponse } from '@fluxer/api/src/user/UserMappers';
	...
	private async performCreateGuild(
		...
		try {
			...
			const guildData: GuildRow = {
				...
				owner_id: user.id,
				name: data.name,
				guild_type: data.guild_type ?? 'server',
			...
			// Always dispatch GUILD_CREATE so gateway has guild/member data for message fetch and subscriptions
			...
			await this.gatewayService.dispatchPresence({
				...
				data: {
					id: guildId.toString(),
					properties: mapGuildToGuildResponse(guild),
					channels: [
						{
							id: textCategoryId.toString(),
							type: 4,
							name: 'Text Channels',
							position: 0,
						...
						{
							id: generalChannelId.toString(),
							type: 0,
							name: 'general',
							parent_id: textCategoryId.toString(),
							position: 0,
							guild_id: guildId.toString()
						},
						{
							id: generalVoiceId.toString(),
							type: 2,
							name: 'General',
							parent_id: voiceCategoryId.toString(),
							position: 0,
							bitrate: 64000,
							guild_id: guildId.toString()
						}
