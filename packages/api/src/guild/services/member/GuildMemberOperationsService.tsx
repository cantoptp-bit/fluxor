...
import {resolveApiErrorCode} from '@fluxer/errors/src/error_handling/ErrorIntrospection';
...
export class GuildMemberOperationsService {
	...
	async getMembers(params: {
		...
		const {userId, guildId, limit = 1, after, requestCache} = params;
		...
		try {
			...
			// When gateway has no responders (e.g. Node gateway), fall back to DB so list still works
			if (
				(error instanceof GatewayRpcMethodError && error.code === GatewayRpcMethodErrorCodes.NO_RESPONDERS) ||
				resolveApiErrorCode(error) === GatewayRpcMethodErrorCodes.NO_RESPONDERS
			) {
				const members = await this.guildRepository.listMembersPaginated(
					guildId,
					limit,
					after,
				);
