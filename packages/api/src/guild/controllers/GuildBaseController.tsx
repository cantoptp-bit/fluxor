...
export function GuildBaseController(app: HonoApp) {
	app.post(
		...
		async (ctx) => {
			...
			const guild = await ctx.get('guildService').createGuild({user, data}, auditLogReason);
			if (
				Config.discovery.enabled &&
				data.guild_type === 'community' &&
				data.list_in_discovery === true &&
				typeof data.discovery_description === 'string' &&
				typeof data.discovery_category === 'number'
			) {
				try {
					await ctx.get('discoveryService').apply({
						guildId: createGuildID(BigInt(guild.id)),
						userId: user.id,
						description: data.discovery_description,
						categoryId: data.discovery_category,
					});
				} catch {
					// Guild was created; discovery application is best-effort (e.g. eligibility)
				}
			}
			return ctx.json(guild);
		},
	);
