...
export class NSFWDetectionService {
	...
	constructor(options?: { modelPath?: string | undefined; nodeEnv?: string | undefined }) {
		...
		const defaultProductionPath = process.platform === 'win32' ? path.join(process.cwd(), 'data', 'model.onnx') : '/opt/data/model.onnx';
		this.modelPath =
			options?.modelPath ??
			(nodeEnv === 'production' ? defaultProductionPath : path.join(process.cwd(), 'data', 'model.onnx'));
	}
	...
	async initialize(): Promise<void> {
		try {
			...
			if (code === 'ENOENT') {
				// Model file missing (e.g. in dev); NSFW checks will be skipped
				this.session = null;
				return;
			}
			throw err;
