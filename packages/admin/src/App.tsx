...
import { isLocalOrPrivateEndpointUrl, normalizeEndpointOrigin, validateOutboundEndpointUrl } from '@fluxer/hono/src/security/OutboundEndpoint';
...
function normalizeAdminSecurityConfig(config: AdminConfig): AdminConfig {
	...
	const allowInsecure = !isProduction || isLiteMode || isLocalOrPrivateEndpointUrl(config.apiEndpoint);

	const apiEndpoint = validateOutboundEndpointUrl(config.apiEndpoint, {
		name: 'admin.apiEndpoint',
		allowHttp: allowInsecure,
		allowLocalhost: allowInsecure,
		allowPrivateIpLiterals: allowInsecure,
	});
