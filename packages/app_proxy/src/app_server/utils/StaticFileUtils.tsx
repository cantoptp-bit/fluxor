...
export function serveSpaFallback(options: ServeSpaFallbackOptions): ServeSpaFallbackResult {
	...
	let indexPath = join(resolvedStaticDir, 'index.html');
	if (!existsSync(indexPath)) {
		// In dev, dist may be empty (cleaned before rspack serve); try source index.html
		const fallbackIndex = resolve(resolvedStaticDir, '..', 'index.html');
