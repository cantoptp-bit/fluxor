...
async function ensureBootstrap(session: cassandra.Client): Promise<void> {
	await session.execute(
		`CREATE KEYSPACE IF NOT EXISTS ${MIGRATION_KEYSPACE} WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1}`,
	);
	await session.execute(
		`CREATE TABLE IF NOT EXISTS ${MIGRATION_KEYSPACE}.${MIGRATION_TABLE} (filename text PRIMARY KEY, applied_at timestamp, checksum text)`,
	);
}
